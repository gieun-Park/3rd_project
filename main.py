...
def format_hashtag(instr:str) -> str:
    return f'<span style="height:20px;line-height:20px;padding:1px 10px 6px 10px;border-radius:9px;display:inline-block;background-color:#9ca3af;color:#fafafa;"># {instr}</span>'
...
def review_card(review_data:dict) -> str:
    ...
    for r in review_data:
        level_code = f"""
                    <p style="margin:0px;">맛&nbsp;&nbsp;&nbsp;{parse_level(r["taste_level"])}&nbsp;&nbsp;&nbsp;가격&nbsp;&nbsp;&nbsp;{parse_level(r["price_level"])}&nbsp;&nbsp;&nbsp;서비스&nbsp;&nbsp;&nbsp;{parse_level(r["service_level"])}</p>""" if parse_level(r["taste_level"]) != "" else ""

        menu_code = f"""
                    <p style="margin:0px;">메뉴: {r["menu"]}</p>""" if is_not_na(r["menu"]) else ""

        tag_code = f"""
                    <p style="margin:10px 0px 0px 0px;">{" ".join([format_hashtag(t) for t in r["tags"]])}</p>""" if len(r["tags"]) != 0 else ""
        ...
        html_code += f"""
            <div style="width:425px;border:0.5px solid #c0c0c0;border-radius:10px;margin-bottom:5px;padding:10px 0px;">
                ...
                <div style="width:425px;display:inline-block;padding:0 15px 0 15px;vertical-align:top;">{level_code}
                    <p style="margin:10px 0px 0px 0px;">⭐{r["score"]}</p>{menu_code}
                    <p style="margin:0px;">{r["content"]}</p>{tag_code}
                </div>
            </div>"""
    return html_code
...
def menu_card(menu_data:list) -> str:
    ...
    for m in menu_data:
        price_code = f"""
                <h6 style="margin:0px;padding:0px;">{int(m["price"])}원</h6>""" if is_not_na(m["price"]) else ""
        description_code = f"""
                <p style="color:gray;margin:5px 0px 0px 0px;">{m["description"]}</p>""" if is_not_na(m["description"]) else ""                
...
def restaurant_page(restaurant_data:dict):
    ...
    open_close_code = f"""
        <p>⏰ {restaurant_data["open_time"]} ~ {restaurant_data["close_time"]}</p>""" if is_not_na(restaurant_data["open_time"]) else ""

    tel_code = f"""
        <p>☎️ {restaurant_data["tel_no"]}</p>""" if is_not_na(restaurant_data["tel_no"]) else ""

    tag_code = f"""
                <p>🏷️ {" ".join([format_hashtag(t) for t in restaurant_data["tags"]])}</p>""" if len(restaurant_data["tags"]) != 0 else ""
...
<div style="{style_main_container}">
    ...
    <div style="{style_container}">
        <h5>홈</h5>
        <p>📍 <strong>{restaurant_data["region"]}</strong> | {restaurant_data["address"]}</p>{open_close_code}{tel_code}{tag_code}
    </div>
    <div style="{style_divider}"></div>
    <div style="{style_container}">
